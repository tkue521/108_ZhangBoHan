...
class InceptionV4(nn.Module):
    ...
    def __init__(self, num_classes = 1001 , init_weights = True):
        super(InceptionV4, self).__init__()
        self.BasicConv2d1 = BasicConv2d(3, 32, kernel_size=3, stride=2)
        self.BasicConv2d2 = BasicConv2d(32, 32, kernel_size=3, stride=1)
        self.BasicConv2d3 = BasicConv2d(32, 64, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.BasicConv2d1(x)
        
        x = self.BasicConv2d2(x)
        
        x = self.BasicConv2d3(x)
        # 147 x 147 x 64
        x = self.Mixed_3a(x)
        # 73 x 73 x 64
        x = self.Mixed_4a(x)
        ...
        #x = nn.AvgPool2d(8, count_include_pad=False)
...
#model = inceptionv4()
#input_image = torch.FloatTensor(100, 3, 299, 299)
